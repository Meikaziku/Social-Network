<footer class="hidden fixed col-span-2 lg:h-full bg-[#C2D3CD] lg:flex lg:flex-col ">

    <div class="flex flex-col gap-12 p-4">
        <form class=" flex justify-center" action="">
            <label class="hidden" for="rechercheAmis"></label>
            <input class="rounded-full bg-white font-[Roboto] px-2 py-1 w-35 xl:w-45 2xl:w-60" type="text" placeholder="Chercher un ami" id="rechercheAmis">
        </form>

        <div class="flex flex-col gap-8 font-[Roboto] ">
            <a href="./accueil.php" class="bg-white rounded-full h-8 lg:flex gap-3 items-center px-2">
                <img class="w-4 h-4" src="./images/icone/house.png" alt="bouton accueil">
                <h3>Accueil</h3>
            </a>

            <a href="./profil.php" class="bg-white rounded-full  h-8 lg:flex gap-3 items-center px-2">
                <img class="w-4 h-4" src="./images/icone/profil.png" alt="bouton profil">
                <h3>Profil</h3>
            </a>
        </div>
    </div>
</footer>